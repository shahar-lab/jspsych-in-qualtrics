  ...
  plugin.trial = function (display_element, trial) {
    ...
    var after_response = function (info) {
      ...
      if (trial.response_ends_trial) {
        if(trial.pre_teardown) {
          setTimeout(() => end_trial(), trial.pre_teardown_duration ? trial.pre_teardown_duration : 1000);
          trial.pre_teardown(c, response);
        }
        else {
          end_trial();
        }
    ...
    if (trial.choices != jsPsych.NO_KEYS) {
      ...
      if(trial.pre_teardown) {
        let stall = trial.pre_teardown_stall ? trial.pre_teardown_stall : 0; 
        setTimeout(() => {
          setTimeout(() => end_trial(), trial.pre_teardown_duration ? trial.pre_teardown_duration : 1500);
          trial.pre_teardown(c);
        }, stall);
      }
    }
